...
function PointCollection() {
  ...
  this.update = function() {
    for (var a = 0; a < this.points.length; a++) {
      var b = this.points[a],
        c = this.mousePos.x - b.curPos.x,
        e = this.mousePos.y - b.curPos.y,
        d = Math.sqrt(c * c + e * e);
      b.targetPos.x = 150 > d ? b.curPos.x - c : b.originalPos.x;
      b.targetPos.y = 150 > d ? b.curPos.y - e : b.originalPos.y;
      b.update()
    }
  };
  this.shake = function() {
    ...
    for (var a = 0; a < this.points.length; a++) {
      var b = this.points[a],
        c = this.mousePos.x - b.curPos.x,
        e = this.mousePos.y - b.curPos.y;
      50 > Math.sqrt(c * c + e * e) && (this.pointCollectionX = Math.floor(5 * Math.random()) - 2, this.pointCollectionY = Math.floor(5 * Math.random()) - 2);
      b.draw(bubbleShape, this.pointCollectionX, this.pointCollectionY)
    }
  };
  this.draw = function(a, b) {
    for (var c = 0; c < this.points.length; c++) {
      var e = this.points[c];
      null !== e && (window.reset && (this.pointCollectionY = this.pointCollectionX = 0, this.mousePos = new Vector(0, 0)), e.draw(a, this.pointCollectionX, this.pointCollectionY, b))
    }
  };
  this.reset = function(a) {}
}
function Point(a, b, c, e, d) {
  this.curPos = new Vector(a, b, c);
  this.color = d;
  this.friction = document.Friction;
  this.rotationForce = document.rotationForce;
  this.springStrength = 0.1;
  this.originalPos = new Vector(a, b, c);
  this.size = this.radius = e;
  this.targetPos = new Vector(a, b, c);
  this.velocity = new Vector(0, 0, 0);
  this.update = function() {
    var a = this.targetPos.x - this.curPos.x,
      b = this.targetPos.y - this.curPos.y,
      c = b * this.springStrength + this.rotationForce * a;
    this.velocity.x += a * this.springStrength - this.rotationForce * b;
    this.velocity.x *= this.friction;
    this.curPos.x += this.velocity.x;
    this.velocity.y += c;
    this.velocity.y *= this.friction;
    this.curPos.y += this.velocity.y;
    a = this.originalPos.x - this.curPos.x;
    b = this.originalPos.y - this.curPos.y;
    a = Math.sqrt(a * a + b * b);
    this.targetPos.z = a / 100 + 1;
    this.velocity.z += (this.targetPos.z - this.curPos.z) * this.springStrength;
    this.velocity.z *= this.friction;
    this.curPos.z += this.velocity.z;
    this.radius = this.size * this.curPos.z;
    1 > this.radius && (this.radius = 1)
  };
  this.draw = function(a, b, c) {
    ctx.fillStyle = this.color;
    "square" == a ? (ctx.beginPath(), ctx.fillRect(this.curPos.x + b, this.curPos.y + c, 1.5 * this.radius, 1.5 * this.radius)) : (ctx.beginPath(), ctx.arc(this.curPos.x + b, this.curPos.y + c, this.radius, 0, 2 * Math.PI, !0), ctx.fill())
  }
}
...
function initEventListeners() {
  ...
  canvas.bind("mousemove", onMove);
  if(_is_mobile){canvas.bind("touchmove", onTouchMove)};
}
...
function updateCanvasDimensions() {
  canvasWidth = (_is_mobile)?parseInt(getCurrentStyle(document.getElementById('myCanvas')).width):(($(window).width() > 1000)?1000:$(window).width());
  canvasHeight = (_is_mobile)?400:700;
  canvas.attr({
    width: canvasWidth,
    height: canvasHeight
  });
...
function onMove(a) {
  pointCollection && pointCollection.mousePos.set(a.pageX - canvas.offset().left, a.pageY - canvas.offset().top);
}
...
function onTouchMove(e) {
  if (e.targetTouches && (e.targetTouches.length == 1)) { 
    ...
    pointCollection && pointCollection.mousePos.set(touch.pageX - canvas.offset().left, touch.pageY - canvas.offset().top);
  }
}
...
function draw(a) {
  var b = canvas.get(0);
  null !== b.getContext && (ctx = b.getContext("2d"), ctx.clearRect(0, 0, canvasWidth, canvasHeight), bubbleShape = "undefined" !== typeof bubbleShape ? bubbleShape : "circle", pointCollection && pointCollection.draw(bubbleShape, a))
}
function shake() {
  var a = canvas.get(0);
  null !== a.getContext && (ctx = a.getContext("2d"), ctx.clearRect(0, 0, canvasWidth, canvasHeight), bubbleShape = "undefined" !== typeof bubbleShape ? bubbleShape : "circle", pointCollection && pointCollection.shake(bubbleShape))
}
function update() {
  pointCollection && pointCollection.update()
}
function drawName(a, b) {
  updateCanvasDimensions();
  for (var c = [], e = 0, d = phraseToHex(a), m = -1, g = 0; g < d.length; g += 2) {
    var h = "A" + d.charAt(g) + d.charAt(g + 1);
    "A20" != h && m++;
    var k = m,
      f = b;
    "undefined" !== typeof f ? ("[object Array]" === Object.prototype.toString.call(f) && "[object Array]" === Object.prototype.toString.call(f[0]) && (b = f), "[object Array]" === Object.prototype.toString.call(f) && "number" === typeof f[0] && (b = [f])) : b = [
      [0, 0, 27]
    ];
    if (document.alphabet.hasOwnProperty(h)) {
      for (var f = document.alphabet[h].P, k = b[k % b.length], l = 0; l < f.length; ++l) {
        point = f[l], c.push(new Point(point[0] + e, point[1], 0, point[2], makeColor(k, point[3])))
      }
      e += document.alphabet[h].W
    }
  }
  for (d = 0; d < c.length; d++) {
    c[d].curPos.x = canvasWidth / 2 - e / 2 + c[d].curPos.x, c[d].curPos.y = canvasHeight / 2 - 105 + c[d].curPos.y, c[d].originalPos.x = canvasWidth / 2 - e / 2 + c[d].originalPos.x, c[d].originalPos.y = canvasHeight / 2 - 105 + c[d].originalPos.y
  }
  pointCollection = new PointCollection;
  pointCollection.points = c;
  initEventListeners()
}
window.reset = !1;
...
var canvas = $("#myCanvas"),
  canvasHeight, canvasWidth, ctx, pointCollection;
